...
fn validate_profile_adapter(profile: &Profile, raw: &Value) -> Result<(), RuntimeError> {
    ...
    if profile.representation.coordinate_adapter_is_biological_ontology == Some(true) {
        return Err(err("coordinate adapter may not be declared biological ontology"));
    }
    ...
    let expected = expected_component_ids();
    ...
    for parameter in &profile.parameters {
        if !names.insert(parameter.name.as_str()) {
            return Err(err(format!("duplicate parameter name: {}", parameter.name)));
        }
        if matches!(parameter.status.as_str(), "observed" | "source-derived" | "calibrated") {
            if parameter.source_id.as_deref().unwrap_or("").is_empty() {
                return Err(err(format!("evidence-backed parameter {} lacks source_id", parameter.name)));
            }
...
fn validate_constraints(profile: &Profile) -> Result<(), RuntimeError> {
    ...
    let ring_participants: BTreeSet<_> = participants(ring)?.into_iter().collect();
    ...
    .into_iter()
    .collect();
    if ring_participants != expected_ring {
        return Err(err("five-sector ring participants do not match V0 adapter"));
    }
    ...
    for participant in &j_participants[1..] {
        if !participant.starts_with("subunit:") {
            return Err(err("J-chain marker may reference only subunits after jchain:0"));
        }
    }
...
///
/// Pentamer sectors use a fixed C5 recurrence. Only the one declared Fab spread
/// angle calls `sin_cos`, once during model construction. No trig or sqrt occurs
/// in the per-work-item hot loop.
pub fn build_geometry(profile: &Profile) -> Result<GeometryState, RuntimeError> {
    ...
    for (sector_index, key) in labels.into_iter().enumerate() {
        ...
        if sector_index != 4 {
            let next_x = C72.mul_add(ux, -S72 * uy);
            let next_y = S72.mul_add(ux, C72 * uy);
            ux = next_x;
            uy = next_y;
        }
    }
...
impl SquaredDistanceGate {
    ...
    pub fn below(self, left: Vec3, right: Vec3) -> bool {
        left.squared_distance(right) < self.cutoff_squared
    }
}
...
pub fn bounded_rotate_z(
    ...
    Ok(Vec3::new(
        pivot.x + c.mul_add(x, -s * y),
        pivot.y + s.mul_add(x, c * y),
        point.z,
    ))
}
...
impl RunConfig {
    pub fn new(work_items: u64, workers: usize, logical_limit: u64) -> Result<Self, RuntimeError> {
        if work_items == 0 || work_items > MAX_WORK_ITEMS || work_items > logical_limit {
            return Err(err("work_items outside runtime/profile logical bound"));
        }
        if workers == 0 || workers > MAX_WORKERS {
            return Err(err("workers outside runtime bound"));
        }
        Ok(Self { work_items, workers })
    }
}
...
fn evaluate_work_item(index: u64, geometry: &GeometryState) -> (u64, f64, f64) {
    ...
    for i in 0..geometry.nodes.len() {
        for j in i + 1..geometry.nodes.len() {
            let d2 = geometry.nodes[i]
                .position
                .squared_distance(geometry.nodes[j].position);
    ...
    for offset in 0..EXECUTION_CELL_STATES {
        ...
        let address = ExecutionAddress::from_sequence(sequence).expect("bounded sequence");
        hash = fnv_update(hash, &[address.sector, address.arm, address.lane]);
        hash = fnv_update(hash, &[address.storage_index().expect("bounded address")]);
    }
    (hash, min_d2, max_d2)
}
...
fn manifest_sha256(profile_sha256: &str, config: RunConfig, ranges: &[WorkRange]) -> String {
    let mut hasher = Sha256::new();
    hasher.update(MANIFEST_DOMAIN);
    hasher.update(profile_sha256.as_bytes());
    hasher.update(config.work_items.to_le_bytes());
...
pub fn run_structural_fixture(loaded: &LoadedProfile, config: RunConfig) -> Result<RunSummary, RuntimeError> {
    let geometry = build_geometry(&loaded.profile)?;
    let ranges = partition_ranges(config.work_items, config.workers)?;
    ...
    let worker_summaries = std::thread::scope(|scope| {
        ...
        for range in ranges.iter().copied() {
            ...
            handles.push(scope.spawn(move || {
                ...
                WorkerSummary {
                    ...
                    pair_checks: pair_count * range.length,
                    crt_microstates: u64::from(EXECUTION_CELL_STATES) * range.length,
        ...
        for handle in handles {
            summaries.push(handle.join().map_err(|_| err("worker thread panicked"))?);
        }
    ...
    let manifest_sha256 = manifest_sha256(&loaded.profile_sha256, config, &ranges);
...
mod tests {
    ...
    fn fixture_profile() -> LoadedProfile {
        load_profile(Path::new("profiles/igm-schematic-pentamer-v0.json"))
            .expect("repository V0 profile must load")
    }
    ...
    fn result_identity_is_worker_independent() {
        ...
        let one = run_structural_fixture(&loaded, RunConfig::new(257, 1, logical).unwrap()).unwrap();
        let seven = run_structural_fixture(&loaded, RunConfig::new(257, 7, logical).unwrap()).unwrap();
    ...
    #[test]
    fn hot_loop_uses_squared_distance_gate() {
        let gate = SquaredDistanceGate::new(2.0).unwrap();
        assert!(gate.below(Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 1.0, 0.0)));
        assert!(!gate.below(Vec3::new(0.0, 0.0, 0.0), Vec3::new(2.0, 0.0, 0.0)));
    }
