...
//! Deterministic Phase 3B PENTA-CRT CPU optimization engine.
//!
//! The engine is admitted only for the synthetic V0 execution profile. C5
//! structure is a computational shortcut, not a biological symmetry claim.
...
pub const OPTIMIZATION_NUMERICAL_PROFILE: &str =
    "IGM-PENTA-CRT-F64-LUT-BLOCK-CIRCULANT-ZRESIDUAL-V1";
...
pub const EXACT_Z_RESIDUAL_CORRECTIONS: usize = 105;
...
#[derive(Debug)]
struct LoadedExecutionProfile {
    profile: ExecutionProfile,
    sha256: String,
}

fn validate_execution_profile(profile: &ExecutionProfile) -> Result<(), RuntimeError> {
    ...
    {
        return Err(err("unsupported Phase 3B execution-profile identity"));
    }
    ...
    {
        return Err(err(
            "Phase 3B requires assumed computational C5 and biological_symmetry_claimed=false",
        ));
    }
    if profile.degrees_of_freedom.len() != DOF_COUNT {
        return Err(err("Phase 3B requires exactly four explicit execution DoFs"));
    }
    ...
    let expected = [
        ...
        (
            "right_fab_delta_deg",
            "degree",
            "all-right-fab-placeholders",
            17usize,
            -45.0_f64,
            45.0_f64,
        ),
        ("jchain_dx", "model-unit", "jchain:0", 9usize, -1.0_f64, 1.0_f64),
        ("jchain_dy", "model-unit", "jchain:0", 9usize, -1.0_f64, 1.0_f64),
    ];
        ...
        profile.degrees_of_freedom.iter().zip(expected)
    {
        ...
        if dof.values.len() != radix {
            return Err(err(format!("{} must declare radix {radix}", dof.id)));
        }
        if !dof
            .values
            .iter()
            .all(|value| value.is_finite() && *value >= lower && *value <= upper)
        {
            return Err(err(format!("{} contains non-finite/out-of-bound value", dof.id)));
        }
        if !dof.values.iter().any(|value| *value == 0.0_f64) {
            return Err(err(format!("{} must include an explicit zero state", dof.id)));
        }
    ...
    if claims.biological_validity_claimed
        || claims.clinical_validity_claimed
        || claims.medical_device_claimed
        || claims.diagnostic_use_claimed
        || claims.treatment_use_claimed
    {
        return Err(err("execution profile may not claim biological/clinical/medical validity"));
    }
    Ok(())
}

fn load_execution_profile(path: &Path) -> Result<LoadedExecutionProfile, RuntimeError> {
...
impl MixedRadix4 {
    fn new(radices: [u16; DOF_COUNT]) -> Result<Self, RuntimeError> {
        if radices.contains(&0) {
            return Err(err("mixed-radix dimensions must be non-zero"));
        }
        let total = radices.iter().try_fold(1_u64, |acc, radix| {
    ...
    fn decode(self, index: u64) -> Result<ConformationAddress, RuntimeError> {
        ...
        let mut value = index;
        let mut digits = [0_u16; DOF_COUNT];
        for (slot, radix) in self.radices.iter().copied().enumerate() {
    ...
    fn encode(self, address: ConformationAddress) -> Result<u64, RuntimeError> {
        ...
        for (digit, radix) in digits.iter().copied().zip(self.radices.iter().copied()) {
            if digit >= radix {
        ...
        for (digit, radix) in digits.iter().copied().zip(self.radices.iter().copied()) {
            index = index
                .checked_add(u64::from(digit) * stride)
...
pub struct PentaCrtEngine {
    model: LoadedProfile,
    execution: LoadedExecutionProfile,
    radix: MixedRadix4,
...
impl PentaCrtEngine {
    pub fn load(model_path: &Path, execution_path: &Path) -> Result<Self, RuntimeError> {
        ...
        if model.profile().model_id != MODEL_ID || model.profile().validation_level != "V0" {
            return Err(err("Phase 3B accepts only the validated V0 schematic model"));
        }
        let execution = load_execution_profile(execution_path)?;
        let mut radices = [0_u16; DOF_COUNT];
        for (index, dof) in execution.profile.degrees_of_freedom.iter().enumerate() {
            radices[index] =
                u16::try_from(dof.values.len()).map_err(|_| err("DoF radix exceeds u16"))?;
        }
        let radix = MixedRadix4::new(radices)?;
        if radix.total > logical_ensemble_size(model.profile())? {
            return Err(err("Phase 3B conformation domain exceeds model logical ensemble"));
        }
        ...
        let make_lut = |values: &[f64]| -> Result<Vec<TrigPair>, RuntimeError> {
            values
                .iter()
                .map(|delta| {
                    deterministic_sin_cos(
                        (base_spread_deg + *delta) * std::f64::consts::PI / 180.0,
                    )
                })
                .collect()
        };
        ...
        let jchain_dx = execution.profile.degrees_of_freedom[2].values.clone();
        let jchain_dy = execution.profile.degrees_of_freedom[3].values.clone();
    ...
    pub fn optimization_profile_sha256(&self) -> &str {
        &self.execution.sha256
    }

    pub fn optimization_profile_id(&self) -> &str {
        &self.execution.profile.profile_id
    }
...
impl GeometrySoa {
    fn empty() -> Self {
        Self {
            x: [0.0_f64; NODE_COUNT],
            y: [0.0_f64; NODE_COUNT],
            z: [0.0_f64; NODE_COUNT],
        }
    }
    ...
    fn set(&mut self, index: usize, point: Vec3) -> Result<(), RuntimeError> {
        if !point.is_finite() {
            return Err(err("Phase 3B geometry produced a non-finite coordinate"));
        }
...
fn build_geometry_with_trig(
    ...
    geometry.set(
        15,
        Vec3::new(
            -engine.jchain_offset + engine.jchain_dx[address.jchain_dx_bin as usize],
            -engine.jchain_offset * V0_JCHAIN_Y_RATIO
                + engine.jchain_dy[address.jchain_dy_bin as usize],
            0.0_f64,
        ),
    )?;
    Ok(geometry)
...
fn reference_geometry(
    ...
    let left_delta = engine.execution.profile.degrees_of_freedom[0].values
        [address.left_fab_bin as usize];
    let right_delta = engine.execution.profile.degrees_of_freedom[1].values
...
fn max_geometry_residual(left: &GeometrySoa, right: &GeometrySoa) -> f64 {
    ...
    for index in 0..NODE_COUNT {
        residual = residual.max((left.x[index] - right.x[index]).abs());
        residual = residual.max((left.y[index] - right.y[index]).abs());
        residual = residual.max((left.z[index] - right.z[index]).abs());
    }
    residual
}
...
fn planar_squared_distance(left: Vec3, right: Vec3) -> Result<f64, RuntimeError> {
    let dx = left.x - right.x;
    let dy = left.y - right.y;
    let d2 = dx * dx + dy * dy;
    if !d2.is_finite() {
        return Err(err("planar squared distance became non-finite"));
    }
    Ok(d2)
}

fn structured_pairs(geometry: &GeometrySoa) -> Result<PairEvaluation, RuntimeError> {
    // Only the XY projection is exactly C5-structured. The legacy V0 Z drawing
    // terms depend on absolute sector angle, so each non-J pair gets its exact
    // local dz^2 residual instead of pretending the full 3D fixture is
    // block-circulant.
    let mut xy_blocks = [[[0.0_f64; 3]; 3]; 5];
    for delta in 0..5_usize {
        for left_local in 0..3_usize {
            for right_local in 0..3_usize {
                xy_blocks[delta][left_local][right_local] = planar_squared_distance(
    ...
    let mut j_distances = [0.0_f64; 15];
    for index in 0..15_usize {
        j_distances[index] = geometry
            .point(index)
            .checked_squared_distance(geometry.point(15))?;
    }
    ...
    for left in 0..NODE_COUNT {
        for right in left + 1..NODE_COUNT {
            let d2 = if right == 15 {
                ...
                let dz = geometry.z[left] - geometry.z[right];
                xy_blocks[delta][left_local][right_local] + dz * dz
            };
            if !d2.is_finite() {
                return Err(err("structured pair reconstruction produced non-finite distance"));
            }
            pairs[cursor] = d2;
            cursor += 1;
...
fn brute_pairs(geometry: &GeometrySoa) -> Result<PairEvaluation, RuntimeError> {
    ...
    for left in 0..NODE_COUNT {
        for right in left + 1..NODE_COUNT {
            let d2 = geometry
                .point(left)
                .checked_squared_distance(geometry.point(right))?;
            pairs[cursor] = d2;
            cursor += 1;
            min_d2 = min_d2.min(d2);
            max_d2 = max_d2.max(d2);
...
fn record_hash(
    ...
    for d2 in pairs {
        if !d2.is_finite() {
            return Err(err("cannot hash non-finite structured pair value"));
        }
...
pub fn verify_penta_crt(
    ...
    if samples == 0
        ...
        || samples as u64 > engine.total_conformations()
    {
    ...
    for sample in 0..samples {
        let index = ((sample as u128 * engine.total_conformations() as u128) / samples as u128)
            as u64;
        let address = engine.decode(index)?;
        let optimized = optimized_geometry(engine, address)?;
        let reference = reference_geometry(engine, address)?;
        max_geometry = max_geometry.max(max_geometry_residual(&optimized, &reference));
        let structured = structured_pairs(&optimized)?;
        let brute = brute_pairs(&reference)?;
        max_pairs = max_pairs.max(max_pair_residual(&structured.pairs, &brute.pairs));
    }
    if !max_geometry.is_finite() || !max_pairs.is_finite() {
        return Err(err("Phase 3B verification residual became non-finite"));
    }
    ...
    Ok(VerificationReport {
        ...
        exact_z_residual_corrections_per_conformation: EXACT_Z_RESIDUAL_CORRECTIONS,
        accepted,
        non_clinical: true,
...
pub fn run_penta_crt(
        ...
        for range in ranges.iter().copied() {
            handles.push(scope.spawn(move || -> Result<PentaCrtWorkerSummary, RuntimeError> {
                ...
                for local_index in range.start..range.end {
                    ...
                    let address = engine.decode(index)?;
                    let evaluation = structured_pairs(&optimized_geometry(engine, address)?)?;
                    diagnostic ^= record_hash(index, address, &evaluation.pairs)?;
                    min_d2 = min_d2.min(evaluation.min_d2);
                    max_d2 = max_d2.max(evaluation.max_d2);
                }
                if !min_d2.is_finite() || !max_d2.is_finite() {
                    return Err(err("PENTA-CRT worker summary became non-finite"));
                }
                Ok(PentaCrtWorkerSummary {
                    ...
                    logical_pair_checks: (PAIR_COUNT as u64)
                        ...
                        .ok_or_else(|| err("worker logical-pair count overflow"))?,
                    planar_block_evaluations: (PLANAR_BLOCK_EVALUATIONS as u64)
                        .checked_mul(range.length)
                        .ok_or_else(|| err("worker planar-block count overflow"))?,
                    sparse_j_evaluations: (SPARSE_J_EVALUATIONS as u64)
                        .checked_mul(range.length)
                        .ok_or_else(|| err("worker sparse-J count overflow"))?,
    ...
    let mut planar_block_evaluations = 0_u64;
    let mut sparse_j_evaluations = 0_u64;
    for worker in &worker_summaries {
        diagnostic ^= u64::from_str_radix(&worker.diagnostic_xor_fnv1a64, 16)
        ...
        logical_pair_checks = logical_pair_checks
            ...
            .ok_or_else(|| err("global logical-pair count overflow"))?;
        planar_block_evaluations = planar_block_evaluations
            .checked_add(worker.planar_block_evaluations)
            .ok_or_else(|| err("global planar-block count overflow"))?;
        sparse_j_evaluations = sparse_j_evaluations
            .checked_add(worker.sparse_j_evaluations)
            .ok_or_else(|| err("global sparse-J count overflow"))?;
    }
    if !min_d2.is_finite() || !max_d2.is_finite() {
        return Err(err("PENTA-CRT global summary became non-finite"));
    }

    let structured_distance_evaluations = planar_block_evaluations
        .checked_add(sparse_j_evaluations)
        .ok_or_else(|| err("structured evaluation count overflow"))?;
    let brute_evaluations = (BRUTE_DISTANCE_EVALUATIONS as u64)
        .checked_mul(config.slice.count)
        .ok_or_else(|| err("brute evaluation count overflow"))?;
    let brute_distance_evaluations_avoided = brute_evaluations
        .checked_sub(structured_distance_evaluations)
        .ok_or_else(|| err("distance-evaluation accounting underflow"))?;
    let exact_z_residual_corrections = (EXACT_Z_RESIDUAL_CORRECTIONS as u64)
        .checked_mul(config.slice.count)
        .ok_or_else(|| err("Z residual correction count overflow"))?;
    ...
    Ok(PentaCrtRunSummary {
        ...
        logical_pair_checks,
        planar_block_evaluations,
        sparse_j_evaluations,
        structured_distance_evaluations,
        brute_distance_evaluations_avoided,
...
mod tests {
    ...
    #[test]
    fn lookup_geometry_matches_reference_projection() {
        let engine = engine();
        for index in [0, 1, 17, 289, 4096, engine.total_conformations() - 1] {
            let address = engine.decode(index).unwrap();
            let optimized = optimized_geometry(&engine, address).unwrap();
            let reference = reference_geometry(&engine, address).unwrap();
            assert!(
                max_geometry_residual(&optimized, &reference)
                    <= BLOCK_REUSE_RESIDUAL_TOLERANCE
            );
        }
    }

    #[test]
    fn structured_xy_reuse_plus_z_residual_matches_brute_force() {
        let engine = engine();
        ...
        assert_eq!(report.exact_z_residual_corrections_per_conformation, 105);
    }
